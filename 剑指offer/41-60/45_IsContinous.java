...
//先对序列也就是扑克牌排序 完了遍历间断不连续的地方 同时统计大小王的个数 可能当做万能牌
// 最后比较大小王的个数是否大于等于间断的距离 也就是万能牌能不能补上不连续的地方
...
//				 出现了对子的情况 无论如何都不能构成顺子
					 return false;
				 }
				 sign += numbers[i + 1] - numbers[i] - 1;
			 }
		 }

		 return count_zeros >= sign;
    }
}
